...
import { getCpuDecision } from "@/lib/cpu.js";
...
import LoginModal from "@/components/Poker/LoginModal.jsx";
...
export default function PokerGame() {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (data.success) {
        ...
        showNotification(
          `✅ Bem-vindo ${username}! Você tem ${data.chips} fichas.`,
          false,
        );
        if (autoSaveRef.current) clearInterval(autoSaveRef.current);
        autoSaveRef.current = setInterval(() => {
          setGame((g) => {
            if (g.handActive && !g.gameOver) {
              saveChips(username, g.playerMoney);
            }
            return g;
          });
        }, 30000);
  ...
  function triggerCpuAction(g, user) {
    ...
    }, 800); // Aumentei para 800ms para dar mais tempo de acompanhar
  }
