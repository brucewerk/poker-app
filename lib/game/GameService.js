...
import { getCpuDecision } from "../poker/cpu.js";
import { getHandRank } from "../poker/evaluation.js";
...
export class GameService {
  ...
  processCPUAction() {
    ...
    // 🔥 CORREÇÃO: Usar a função getCpuDecision corrigida
    const cpuDecision = getCpuDecision(
      this.gameState,
      this.advanceRound.bind(this),
      (msg) => console.log(msg), // showNotification placeholder
      "cpu" // user placeholder
    );
  ...
  executeCPUAction(decision) {
    // 🔥 CORREÇÃO: Se decision já for um estado completo (do getCpuDecision), use-o diretamente
    if (decision.cpuMoney !== undefined && decision.pot !== undefined) {
      // Atualizar o estado do jogo com o resultado da decisão da CPU
      this.gameState = {
        ...this.gameState,
        ...decision
      };
      return { action: "cpu", message: decision.gameStatus || "CPU agiu" };
    }

    // Caso contrário, processe como decisão individual (formato antigo)
   ...
   */
  showdown() {
    const playerHand = getHandRank(
      this.gameState.playerCards,
      this.gameState.communityCards,
    );
    const cpuHand = getHandRank(
    ...
    // 🔥 CORREÇÃO: Usar score em vez de value para comparação
    const playerScore = playerHand.score || playerHand.raw || 0;
    const cpuScore = cpuHand.score || cpuHand.raw || 0;
