...
export function getCpuDecision(state, advanceStage, showNotification, user) {
  ...
  const strength = calculateHandStrength(normalizedCpuCards, normalizedCommunity);
  const bigBlind = 50;

  // NOVA: Avaliar mão atual para decisões mais precisas
  const handEvaluation =
    normalizedCommunity.length >= 3
      ? getHandRank(normalizedCpuCards, normalizedCommunity)
      : null;
