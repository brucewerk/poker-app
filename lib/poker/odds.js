...
function countOuts(playerCards, communityCards) {
  // 🔥 CORREÇÃO: Normalizar cartas para garantir compatibilidade
  ...
  const normalizedCommunityCards = communityCards.map(card => ({
    ...card,
    rank: card.value || card.rank,
    value: card.value || card.rank
  }));
  
  const allCards = [...normalizedPlayerCards, ...normalizedCommunityCards];
