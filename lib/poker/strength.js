...
export function calculateHandStrength(holeCards, communityCards) {
  ...
  const isPair = (holeCards[0].value || holeCards[0].rank) === (holeCards[1].value || holeCards[1].rank);
  ...
  const low = Math.min(holeCards[0].value || holeCards[0].rank, holeCards[1].value || holeCards[1].rank);
  ...
  // Verificar cartas com valor ao invés de rank para compatibilidade
  const card1Value = holeCards[0].value || holeCards[0].rank;
  const card2Value = holeCards[1].value || holeCards[1].rank;
  
  // Ajuste para cartas com valores numéricos
  if (typeof card1Value === 'number' && typeof card2Value === 'number') {
    const maxVal = Math.max(card1Value, card2Value);
    const minVal = Math.min(card1Value, card2Value);
    
    // Se tiver cartas altas numéricas
    if (maxVal >= 10 && minVal >= 8) {
      if (isSuited) return 0.55;
      return 0.45;
    }
  }
...
function calculateDrawStrength(holeCards, communityCards) {
  ...
  const ranks = allCards.map((c) => c.value || c.rank);
