...
// 🔥 MÉTODO PARA ADICIONAR AO HISTÓRICO (CORRIGIDO - SEM DUPLICATAS)
UserSchema.methods.addHandHistory = async function (handData) {
  ...
  const handId =
    handData.id ||
    `${this.username}_${Date.now()}_${Math.random().toString(36).substr(2, 6)}`;
  ...
  // 🔥 VERIFICAR POR DUPLICATA (ID)
  const existingIndex = this.handHistory.findIndex((h) => h.id === handId);

  if (existingIndex !== -1) {
    // ATUALIZAR EXISTENTE
    this.handHistory[existingIndex] = handToSave;
    console.log(`🔄 [HISTORY] Model: Entrada ${handId} atualizada`);
  } else {
    // 🔥 VERIFICAR POR DUPLICATA (TIMESTAMP + RESULTADO + POTE)
    const existsByData = this.handHistory.some(
      (h) =>
    ...
    if (!existsByData) {
      ...
      console.log(`✅ [HISTORY] Model: Nova entrada ${handId} adicionada`);
    } else {
      console.log(`⚠️ [HISTORY] Model: Duplicata ignorada`);
    }
  }
